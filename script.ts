import {
  TezosBTC,
  UnsignedTezosTransaction,
} from '@airgap/coinlib-core'

async function main(): Promise<void> {
  try {
    const protocol = new TezosBTC()

    const unsignedTx: UnsignedTezosTransaction = {
      transaction: {
        binaryTransaction:
          '',
      },

      publicKey:
        '9430c2ac8fe1403c6cbbee3a98b19f3f3bbdd89d0659b3eb6e4106a5cbe41351',

      callback: 'airgap-wallet://?d=',
    }

    console.log('Protocol:', protocol.constructor.name)

    const airGapTxs = await protocol.getTransactionDetails(unsignedTx)

    console.log('Transaction Details:', airGapTxs)
  } catch (error) {
    console.error('Failed to fetch transaction details:', error)
  }
}

void main()
